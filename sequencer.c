...
static int parse_head(struct commit **head)
{
	...
	if (get_oid("HEAD", &oid)) {
		...
		current_head = lookup_commit_reference(oid.hash);
		if (!current_head)
...
{
	int edit = 1, amend = 0, cleanup_commit_message = 0;
	...
	if (file_exists(rebase_path_amend())) {
		...
		if (is_clean && !hashcmp(head, to_amend) &&
			...
			opts->current_fixup_count--;
			if (!len)
				die("BUG: Incorrect current_fixups:\n%s", p);
			while (len && p[len - 1] != '\n')
				len--;
	...
	if (run_git_commit(final_fixup ? NULL : rebase_path_message(),
			   opts, 1, edit, amend, cleanup_commit_message))
