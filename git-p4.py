...
def gitBranchExists(branch):
    ...
    sys.stderr.write('running cmd %s as pid %d\n' % (str(["git", "rev-parse", branch]), proc.pid))
    return proc.wait() == 0;
...
class GitLFS(LargeFileSystem):
    ...
    def generatePointer(self, contentFile):
        ...
        sys.stderr.write('running cmd %s as pid %d\n' % (str(['git', 'lfs', 'pointer', '--file=' + contentFile]), pointerProcess.pid))
    ...
    def pushFile(self, localLargeFile):
        ...
        sys.stderr.write('running cmd %s as pid %d\n' % (str(['git', 'lfs', 'push', '--object-id', 'origin', os.path.basename(localLargeFile)]), uploadProcess.pid))
...
class P4Sync(Command, P4UserMap):
    ...
    def openStreams(self):
        self.importProcess = subprocess.Popen(["git", "fast-import"],
                                              ...
                                              stdout=subprocess.PIPE,
                                              stderr=subprocess.PIPE);
        sys.stderr.write('running cmd %s as pid %d\n' % (str(["git", "fast-import"]), self.importProcess.pid))
